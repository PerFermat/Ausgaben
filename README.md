...
- **Arithmetic in the amount field**: `10+20*3` → 70, `100,50*2+25` → 226; only **addition (`+`) and
  multiplication (`*`)** are allowed, with usual precedence (× before +) and one decimal mark per number.
  Subtraction, division, brackets and functions are **not** possible; invalid characters cannot even be
  typed, and incomplete/invalid calculations are flagged with a message. The field evaluates on blur and
  shows the result. Applies to the partial amounts of a split booking and the value quick-entry from the
  main menu too.
...
- **Categories** (menu **⋮ → Categories**, also reachable from the portfolio): answers “Where does my money
  go?” – the **expenses per category** as a **pie chart** (full width, at most half the screen height,
  unlabelled slices, a tap shows category and amount in the centre) plus a **descending list** with the share
  in percent that scrolls on its own – the chart stays visible. Switchable **month/year**; in **both** views
  a **swipe** pages through the periods (like in the budget), with a header showing previous | **current** |
...
- **Scheduled transactions** (own menu page, shown **only in `.kmy` mode**): imports the standing orders /
  ...
  **yellow** = transfer; the date is its own column, the **name** with the **payee** behind it and the
...
- **KMyMoney `.kmy` mode**: writes new bookings straight into the `.kmy` (gzip XML) and imports
  ...
  stock/ETF) are imported as a **transfer** into/out of the security. Any **fee/tax** on it does not appear
  in the booking list and changes no balances, but is counted as an expense in the **category evaluations**
  (Categories page, Budget) via a separate, hidden store; dividends stay income. Import replaces the
  already-exported bookings per account. Before every write-back a **timestamped backup** of the `.kmy`
